...
function FileMessage({ message, document = false, onOpen }) {
  ...
  return <div className="flex items-center gap-2 overflow-hidden rounded-xl bg-black/[.1] p-2">
    ...
    <span className="flex gap-1">
      {previewable && <button className="grid h-7 w-7 place-items-center rounded-full hover:bg-text-primary/[.08]" type="button" onClick={onOpen} aria-label={`Preview ${name}`}><FiEye /></button>}
      {message.mediaUrl && <a className="grid h-7 w-7 place-items-center rounded-full hover:bg-text-primary/[.08]" href={message.mediaUrl} download={name} aria-label={`Download ${name}`} onClick={(event) => event.stopPropagation()}><FiDownload /></a>}
    </span>
  </div>;
}
